...
def levenshtein_similarity(text_1, text_2, ignore_space=True):
	...
	"""
	numerator = levenshtein_distance(text_1, text_2, ignore_space=ignore_space)
...
def damerau_levenshtein_distance(text_1, text_2, ignore_space=True):
	"""
	Get Damerau-Levenshtein distance between two lines of text.
	"""
	s1 = text_1.replace(' ', '').lower() if ignore_space else text_1.lower()
	s2 = text_2.replace(' ', '').lower() if ignore_space else text_2.lower()
	len_1, len_2 = len(s1), len(s2)
	...
	for i in range(1,len_1+1):
		for j in range(1, len_2+1):
			cost_1 = 1 if s1[i-1] != s2[j-1] else 0
			if i >= 2 and j >= 2:
				cost_2 = 1 if s1[i-2] == s2[j-1] and s1[i-1] == s2[j-2] else 9527
...
def damerau_levenshtein_similarity(text_1, text_2, ignore_space=True):
	"""
	Get Damerau-Levenshtein similarity between two lines of text.
	"""
	numerator = damerau_distance(text_1, text_2, ignore_space=ignore_space)
	denominator = max(len(text_1.replace(' ', '')),len(text_2.replace(' ', ''))) if ignore_space else max(len(text_1),len(text_2))
	similarity = 1-numerator/denominator
